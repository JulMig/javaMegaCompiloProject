...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public boolean compatibleWith(Type _other) {
		...
		if (_other instanceof RecordType otherType) {
			return this.getName().equals(otherType.getName());
			
		} else if (_other instanceof SequenceType otherType) {
			return otherType.compatibleWith(this);
		} else {
			return false;
		}
	...
	/*
	 * Permet de renseigner l'offset des champs
	 */
	public int setFieldsOffsets(int offset) {
		for (FieldDeclaration field : fields) {
			offset += field.computerOffset(offset);
		}
